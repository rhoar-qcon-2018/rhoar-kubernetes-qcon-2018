...
----
    ...
    public void getREST(Handler<AsyncResult<JsonObject>> insultGetHandler) {
        ...
        CompositeFuture
        .all(getNoun(), getAdjective(), getAdjective())         {conum-guard-groovy} <1>
            .rxSetHandler()
            .flatMapMaybe(InsultServiceImpl::mapResultToError)  {conum-guard-groovy} <2>
            .map(InsultServiceImpl::buildInsult)                {conum-guard-groovy} <3>
            .subscribe(
                fut::complete,                                  {conum-guard-groovy} <4>
                fut::fail                                       {conum-guard-groovy} <5>
            );
    }
