...
<<<
== Exercise 2: Create A New Vert.x Project
. Ensure that you have Apache Maven >= 3.3.9
. Create an `insult-service` directory underthe `rhoar-kubernetes-qcon-2018` top-level directory
. Change into the new `insult-service` directory
. Run the following command

.Executing Fabric8 Vert.x Plugin To Start A New Project
[source,bash]
----
$ mvn io.reactiverse:vertx-maven-plugin:1.0.13:setup -DvertxVersion=3.5.1
