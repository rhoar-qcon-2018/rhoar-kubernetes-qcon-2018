...
class MainVerticleSpec extends Specification {
    ...
    def 'Test Vert.x configuration loading'() {
        given: 'An instance of Vert.x'  {conum-guard-js} <1>
            def vertx = Vertx.vertx()
        and: 'An instance of a Vert.x Future'  {conum-guard-js} <2>
            def fut = Future.future()
        and: '''An instance of Spock's AsyncConditions''' {conum-guard-js} <3>
            def async = new AsyncConditions<1>

        when: 'We attempt to deploy the main Verticle'  {conum-guard-js} <4>
            vertx.deployVerticle(new MainVerticle(), fut.completer())

        then: 'Expect that the correct configuration is found and loaded'
            fut.setHandler({ res ->
                async.evaluate { {conum-guard-js} <5>
                    res.succeeded() {conum-guard-js} <6>
                    vertx.getOrCreateContext().config().hasProperty('noun') {conum-guard-js} <7>
                    vertx.getOrCreateContext().config().hasProperty('adjective') {conum-guard-js} <8>
                    vertx.getOrCreateContext().config().hasProperty('http') {conum-guard-js} <9>
                }
            })

        cleanup: 'Await the async operations'  {conum-guard-js} <10>
            async.await(10)
    }
}
----
...
public class MainVerticle extends AbstractVerticle {
    ...
    Single<JsonObject> initConfigRetriever() {                      {conum-guard-js} <1>
        // Load the default configuration from the classpath
        LOG.info("Configuration store loading.");
        ConfigStoreOptions defaultOpts = new ConfigStoreOptions()   {conum-guard-js} <2>
                .setType("file")
                .setFormat("json")
        ...
        ConfigStoreOptions localConfig = new ConfigStoreOptions()   {conum-guard-js} <3>
                .setType("file")
                .setFormat("json")
        ...
        ConfigRetrieverOptions retrieverOptions = new ConfigRetrieverOptions()
                ...
                .addStore(localConfig); {conum-guard-js} <4>

        // Check to see if we are running on Kubernetes/OCP
        if (System.getenv().containsKey("KUBERNETES_NAMESPACE")) {  {conum-guard-js} <5>

            // When running inside of Kubernetes, configure the application to also load from a ConfigMap
            ConfigStoreOptions confOpts = new ConfigStoreOptions()  {conum-guard-js} <6>
                    .setType("configmap")
                    .setConfig(new JsonObject()
                            .put("name", "insult-config")
                            .put("optional", true)
                    );
            retrieverOptions.addStore(confOpts);                    {conum-guard-js} <7>
        }
    ...
    public void start(Future<Void> startFuture) {
        ...
        initConfigRetriever()   {conum-guard-js} <8>
                .doOnError(startFuture::fail)                       {conum-guard-js} <9>
                .subscribe(c -> {
                    LOG.info(c.encodePrettily());
                    context.config().mergeIn(c);                    {conum-guard-js} <10>
                    startFuture.complete();                         {conum-guard-js} <11>
                });
    }
}
----
