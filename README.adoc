...
public class MainVerticle extends AbstractVerticle {
    ...
    Maybe<JsonObject> initConfigRetriever() {                                   {conum-guard-groovy} <1>
        ...
        ConfigStoreOptions confOpts = new ConfigStoreOptions()                  {conum-guard-groovy} <3>
                .setType("configmap")
        ...
        ConfigRetrieverOptions retrieverOptions = new ConfigRetrieverOptions()  {conum-guard-groovy} <4>
        ...
        return ConfigRetriever.create(vertx, retrieverOptions)
                .rxGetConfig().toMaybe();                                       {conum-guard-groovy} <5>
    }

    @Override
    public void start(Future<Void> startFuture) {

        initConfigRetriever()                                                   {conum-guard-groovy} <6>
                .doOnError(startFuture::fail)                                   {conum-guard-groovy} <7>
                .subscribe(c -> {
                    LOG.info(c.encodePrettily());
                    context.config().mergeIn(c);                                {conum-guard-groovy} <8>
                    startFuture.complete();                                     {conum-guard-groovy} <9>
                });
    }
}
----
<1> Define a new method which returns a `Maybe` with the configuration
<2> Create an instance of `ConfigStoreOptions` to load the default config from the classpath
<3> Create an instance of `ConfigStoreOptions` to load configuration data from Kubernetes ConfigMaps
<4> Attach the `ConfigStoreOptions` to the `ConfigRetrieverOptions`
<5> Return the RxJava2 `Maybe` which may be completed at a later time
<6> From inside of the `start` method, call `initConfigRetriever`
<7> Set an error handler for the `Single` which will fail the Verticle deployment when an error is encountered
<8> Merge the loaded configuration into the global Vert.x configuration
<9> Complete the `startFuture` successfully
...
the Kubernetes ConfigMap value will take precedence.
====
...
Now we can run our first test with maven by executing `mvn clean package`. We should see the initial
test run fail because we have not yet implemented the required features. [big red]#RED#, perfect!
...
public class InsultServiceImpl implements InsultService {
     ...
     */
    @Override
    public void publish(Handler<AsyncResult<JsonObject>> handler) {
        insultGetHandler.handle(Future.failedFuture(new Exception("Not Implemented")));
    }
}
----
...
For more simple interactions with HTTP servers, the Vert.x team has provided the `vertx-web-client` library. For
interacting with the other microservices via REST, we will use the web client. The simple case of using the web client
...
client.get(8080, "localhost", "/some/path")
        .timeout(3000)
        .rxSend()
...
isolation. Instead, we can extract those into stateless methods which are simple to test and read.

.Web Client Example
[source,java,subs=attributes+]
----
private HttpResponse<Buffer> mapErrors(HttpResponse<Buffer> resp) throws HttpResponseException {
...
private void example(Future<JsonObject> httpResponse) {
    ...
    client.get(8080, "localhost", "/some/path")
            ...
            .map(this::mapErrors)
            .map(HttpResponse::bodyAsJsonObject)
            .subscribe(httpResponse::complete, httpResponse::fail);
}
----

This code is simple to test and easier to read. As we go forward, this is the pattern I will recommend whenever
...
These futures can then be *_composed_* in another method using *_CompositeFuture_*. CompositeFuture allows us to have
multiple operations run asynchronously and in parallel.
...
----
    ...
    public void getREST(Handler<AsyncResult<JsonObject>> insultGetHandler) {
        ...
        CompositeFuture
        .all(getNoun(), getAdjective(), getAdjective())         {conum-guard-java} <1>
            .rxSetHandler()
            .flatMapMaybe(InsultServiceImpl::mapResultToError)  {conum-guard-java} <2>
            .map(InsultServiceImpl::buildInsult)                {conum-guard-java} <3>
            .subscribe(
                fut::complete,                                  {conum-guard-java} <4>
                fut::fail                                       {conum-guard-java} <5>
            );
    }

    -- SNIP --
----
<1> Launch 3 parallel REST requests
<2> Map errors to an exception
<3> Combine the 3 results into a single JSON object
<4> Map successful JSON to a succeeded future
<5> When an exception happens, map it to a failed future
...
public class InsultServiceImpl implements InsultService {
    ...
    public InsultServiceImpl(io.vertx.core.Vertx vertx, JsonObject config) {
        ...
        kafka = KafkaService.createProxy(Vertx.newInstance(vertx), "kafka.service");    {conum-guard-java} <1>
    ...
    public InsultService publish(JsonObject insult, Handler<AsyncResult<Void>> handler) {
        Future<Void> fut = Future.future();             {conum-guard-java} <2>
        fut.setHandler(handler);                        {conum-guard-java} <3>
        kafka.rxPublish(insult)
             .subscribe(fut::complete, fut::fail);      {conum-guard-java} <4>
        return this;
    }
...
<4> Handle both success and failure for the stream
