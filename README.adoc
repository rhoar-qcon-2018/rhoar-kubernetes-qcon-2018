...
class MainVerticleSpec extends Specification {
    ...
    def 'Test Vert.x configuration loading'() {
        ...
        and: '''An instance of Spock's AsyncConditions''' // <3>
            def async = new AsyncConditions<1>

        when: 'We attempt to deploy the main Verticle'  // <4>
        ...
        then: 'Expect that the correct configuration is found and loaded'
            fut.setHandler({ res ->
                async.evaluate { // <5>
                    res.succeeded() // <6>
                    vertx.getOrCreateContext().config().hasProperty('noun') // <7>
                    vertx.getOrCreateContext().config().hasProperty('adjective') // <8>
                    vertx.getOrCreateContext().config().hasProperty('http') // <9>
                }
            })
...
public class MainVerticle extends AbstractVerticle {
    ...
    Single<JsonObject> initConfigRetriever() {                      // <1>
    ...
    public void start(Future<Void> startFuture) {
        ...
        initConfigRetriever()   // <8>
                .doOnError(startFuture::fail)                       // <9>
                .subscribe(c -> {
                    LOG.info(c.encodePrettily());
                    context.config().mergeIn(c);                    // <10>
                    startFuture.complete();                         // <11>
                });
    }
}
----
