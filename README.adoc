...
==== Additional Dependencies When Running Locally With MiniShift
. VirtualBox
. >= 16GB RAM
...
=== Execute Inventory On Remote OpenShift Cluster
...
=== Execute Inventory On Local MiniShift Instance

In order to run minishift locally, you will need a Red Hat Developers account. You can get one by going https://developers.redhat.com/[HERE].
...
refer to the Appendix <<Create A New Vert.x Project Using The Vert.x Maven Plugin>>
====
...
http://spockframework.org/spock/docs/1.1/index.html[SpockFramework]. In order to use Spock, we need a few dependencies which have already been added in the skeleton:
...
With the latest versions of Jenkins, the new https://jenkins.io/doc/book/pipeline/syntax/[*_declarative pipeline syntax_*] has become the preferred way in which to define DevOps pipelines. The skeleton project you checked out earlier has a very complete Jenkins pipeline which we will discuss in detail as we go along.
...
*_CI/CD_* project and click on *_Add to project -> Select from project_* in the top right corner of the OpenShift web console.
